        ...
        for(unsigned q = 0; q < phi.n_q_points; ++q) {
          ...
          for(unsigned s = 1; s <= IMEX_stage - 1; ++s) {
            ...
            flux += b[s - 1]*dt*(rho_s*tensor_product_u_s) +
                    b_tilde[s - 1]*dt*(p_prime_s_times_identity*inv_Ma2);
          }
