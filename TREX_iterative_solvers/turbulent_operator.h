...
namespace Turbulent_Diffusivity {
  ...
  public:
    ...
    void set_TRBDF2_stage(const unsigned int stage); /*--- Setter of the TR-BDF2 stage. ---*/
  ...
  protected:
    ...
    unsigned int TRBDF2_stage; /*--- Flag for the TR-BDF2 stage ---*/
  ...
  void TurbulentOperator<dim, fe_degree_T, fe_degree_u,
                              n_q_points_1d, n_q_points_1d_boundary, Vec>::
  set_TRBDF2_stage(const unsigned int stage) {
    AssertIndexRange(stage, EquationData::n_stages + 1);
  ...
  assemble_rhs_cell_term_velocity(const MatrixFree<dim, Number>&               data,
    ...
    if(TRBDF2_stage == 2) {
      ...
      FEEvaluation<dim, fe_degree_T, n_q_points_1d, 1, Number>   phi_theta_curr(data, EquationData::THETA_INDEX_DOF);
      ...
      FEEvaluation<dim, fe_degree_T, n_q_points_1d, 1, Number>   phi_theta_curr(data, EquationData::THETA_INDEX_DOF),
                                                                 phi_theta_s_2(data, EquationData::THETA_INDEX_DOF);
      ...
      for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
        ...
        phi_u_s_2.gather_evaluate(src[2], EvaluationFlags::gradients);
        phi_theta_s_2.reinit(cell);
        phi_theta_s_2.gather_evaluate(src[3], EvaluationFlags::gradients);
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          const auto& grad_u_s_2              = phi_u_s_2.get_gradient(q);
          const auto& grad_theta_s_2          = phi_theta_s_2.get_gradient(q);
          const auto& mod_squared_grad_uz_s_2 = grad_u_s_2[0][dim - 1]*grad_u_s_2[0][dim - 1];
          const auto& Ri_s_2                  = 1.0/(EquationData::Fr2)*grad_theta_s_2[dim - 1]/mod_squared_grad_uz_s_2;
          for(unsigned int idx = 0; idx < VectorizedArray<Number>::size(); ++idx) {
            if(Ri_s_2[idx] > 0.0) {
              b[idx]    = 5.0;
              beta[idx] = -2.0;
          ...
          const auto& kappa_s_2 = EquationData::l_mixing*EquationData::l_mixing*std::sqrt(mod_squared_grad_uz_s_2)*
                                  std::pow(1.0 + b*std::abs(Ri_s_2), beta);
          Tensor<2, dim, VectorizedArray<Number>> diff_flux_s_2;
          diff_flux_s_2 = 0;
          diff_flux_s_2[0][dim - 1] = kappa_s_2*grad_u_s_2[0][dim - 1];

          phi.submit_value(u_curr, q);
          phi.submit_gradient(-a31_tilde*dt*diff_flux_curr
                              -a32_tilde*dt*diff_flux_s_2, q);
        }
  ...
  assemble_rhs_face_term_velocity(const MatrixFree<dim, Number>&               data,
    ...
    if(TRBDF2_stage == 2) {
      ...
      FEFaceEvaluation<dim, fe_degree_T, n_q_points_1d, 1, Number>   phi_theta_curr_m(data, true, EquationData::THETA_INDEX_DOF),
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        const auto coef_jump = C_u*0.5*(std::abs((phi_m.get_normal_vector(0) * phi_m.inverse_jacobian(0))[dim - 1]) +
        ...
        for(unsigned int q = 0; q < phi_m.n_q_points; ++q) {
          ...
          avg_diff_flux_curr[0][dim - 1] = 0.5*(kappa_curr_m*grad_u_curr_m[0][dim - 1] +
                                                kappa_curr_p*grad_u_curr_p[0][dim - 1]);
          ...
          const auto& avg_kappa_curr = 2.0/(1.0/kappa_curr_m + 1.0/kappa_curr_p);
          auto jump_u_curr           = u_curr_m - u_curr_p;
          jump_u_curr[dim - 1] = make_vectorized_array<Number>(0.0);

          phi_m.submit_value(a21_tilde*dt*(avg_diff_flux_curr*n_minus -
                                           coef_jump*avg_kappa_curr*jump_u_curr), q);
          phi_p.submit_value(-a21_tilde*dt*(avg_diff_flux_curr*n_minus -
                                            coef_jump*avg_kappa_curr*jump_u_curr), q);
        }
      ...
      FEFaceEvaluation<dim, fe_degree_T, n_q_points_1d, 1, Number>   phi_theta_curr_m(data, true, EquationData::THETA_INDEX_DOF),
                                                                     phi_theta_curr_p(data, false, EquationData::THETA_INDEX_DOF),
                                                                     phi_theta_s_2_m(data, true, EquationData::THETA_INDEX_DOF),
                                                                     phi_theta_s_2_p(data, false, EquationData::THETA_INDEX_DOF);
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        phi_u_s_2_m.gather_evaluate(src[2], EvaluationFlags::values | EvaluationFlags::gradients);
        phi_u_s_2_p.reinit(face);
        phi_u_s_2_p.gather_evaluate(src[2], EvaluationFlags::values | EvaluationFlags::gradients);
        phi_theta_s_2_m.reinit(face);
        phi_theta_s_2_m.gather_evaluate(src[3], EvaluationFlags::gradients);
        phi_theta_s_2_p.reinit(face);
        phi_theta_s_2_p.gather_evaluate(src[3], EvaluationFlags::gradients);
        ...
        for(unsigned int q = 0; q < phi_m.n_q_points; ++q) {
          ...
          /*--- Compute contribution at previous stage ---*/
          const auto& grad_u_s_2_m              = phi_u_s_2_m.get_gradient(q);
          const auto& grad_u_s_2_p              = phi_u_s_2_p.get_gradient(q);
          const auto& grad_theta_s_2_m          = phi_theta_s_2_m.get_gradient(q);
          const auto& grad_theta_s_2_p          = phi_theta_s_2_p.get_gradient(q);
          const auto& mod_squared_grad_uz_s_2_m = grad_u_s_2_m[0][dim - 1]*grad_u_s_2_m[0][dim - 1];
          const auto& mod_squared_grad_uz_s_2_p = grad_u_s_2_p[0][dim - 1]*grad_u_s_2_p[0][dim - 1];
          const auto& Ri_s_2_m                  = 1.0/(EquationData::Fr2)*grad_theta_s_2_m[dim - 1]/mod_squared_grad_uz_s_2_m;
          const auto& Ri_s_2_p                  = 1.0/(EquationData::Fr2)*grad_theta_s_2_p[dim - 1]/mod_squared_grad_uz_s_2_p;
          for(unsigned int idx = 0; idx < VectorizedArray<Number>::size(); ++idx) {
            if(Ri_s_2_m[idx] > 0.0) {
              b_m[idx]    = 5.0;
              beta_m[idx] = -2.0;
          ...
          const auto& kappa_s_2_m = EquationData::l_mixing*EquationData::l_mixing*std::sqrt(mod_squared_grad_uz_s_2_m)*
                                    std::pow(1.0 + b_m*std::abs(Ri_s_2_m), beta_m);
          const auto& kappa_s_2_p = EquationData::l_mixing*EquationData::l_mixing*std::sqrt(mod_squared_grad_uz_s_2_p)*
                                    std::pow(1.0 + b_p*std::abs(Ri_s_2_p), beta_p);
          Tensor<2, dim, VectorizedArray<Number>> avg_diff_flux_s_2;
          avg_diff_flux_s_2 = 0;
          avg_diff_flux_s_2[0][dim - 1] = 0.5*(kappa_s_2_m*grad_u_s_2_m[0][dim - 1] +
                                               kappa_s_2_p*grad_u_s_2_p[0][dim - 1]);
          ...
          const auto& avg_kappa_s_2  = 2.0/(1.0/kappa_s_2_m + 1.0/kappa_s_2_p);
          auto jump_u_s_2            = u_s_2_m - u_s_2_p;
          jump_u_s_2[dim - 1]        = make_vectorized_array<Number>(0.0);

          phi_m.submit_value(a31_tilde*dt*(avg_diff_flux_curr*n_minus -
                                           coef_jump*avg_kappa_curr*jump_u_curr) +
                             a32_tilde*dt*(avg_diff_flux_s_2*n_minus -
                                           coef_jump*avg_kappa_s_2*jump_u_s_2), q);
          phi_p.submit_value(-a31_tilde*dt*(avg_diff_flux_curr*n_minus -
                                            coef_jump*avg_kappa_curr*jump_u_curr)
                             -a32_tilde*dt*(avg_diff_flux_s_2*n_minus -
                                            coef_jump*avg_kappa_s_2*jump_u_s_2), q);
        }
  ...
  assemble_cell_term_velocity(const MatrixFree<dim, Number>&               data,
    ...
    const double coeff = (TRBDF2_stage == 2) ? a22_tilde*dt : a33_tilde*dt;
  ...
  assemble_face_term_velocity(const MatrixFree<dim, Number>&               data,
    ...
    for(unsigned int face = face_range.first; face < face_range.second; ++face) {
      ...
      for(unsigned int q = 0; q < phi_m.n_q_points; ++q) {
        ...
        avg_diff_flux[0][dim - 1] = 0.5*(kappa_curr_m*grad_u_m[0][dim - 1] +
        ...
        phi_m.submit_value(coeff*(-avg_diff_flux*n_minus + coef_jump*avg_kappa_curr*jump_u), q);
        phi_p.submit_value(-coeff*(-avg_diff_flux*n_minus + coef_jump*avg_kappa_curr*jump_u), q);
      }
  ...
  assemble_rhs_cell_term_temperature(const MatrixFree<dim, Number>&               data,
    ...
    if(TRBDF2_stage == 2) {
      /*--- We start by declaring suitable instances to read the available quantities ---*/
      FEEvaluation<dim, fe_degree_T, n_q_points_1d, 1, Number>   phi(data, EquationData::THETA_INDEX_DOF),
                                                                 phi_theta_curr(data, EquationData::THETA_INDEX_DOF);
      FEEvaluation<dim, fe_degree_u, n_q_points_1d, dim, Number> phi_u_curr(data, EquationData::U_INDEX_DOF);
      ...
      for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          const auto& kappa_s_2 = EquationData::l_mixing*EquationData::l_mixing*std::sqrt(mod_squared_grad_uz_s_2)*
                                  std::pow(1.0 + b*std::abs(Ri_s_2), beta);
          Tensor<2, dim, VectorizedArray<Number>> diff_tensor_s_2;
          diff_tensor_s_2 = 0;
          diff_tensor_s_2[dim - 1][dim - 1] = kappa_s_2;

          phi.submit_value(theta_curr, q);
          phi.submit_gradient(-a31_tilde*dt*diff_tensor_curr*grad_theta_curr
                              -a32_tilde*dt*diff_tensor_s_2*grad_theta_s_2, q);
        }
  ...
  assemble_rhs_face_term_temperature(const MatrixFree<dim, Number>&               data,
    ...
    if(TRBDF2_stage == 2) {
      /*--- We start by declaring suitable instances to read the available quantities ---*/
      FEFaceEvaluation<dim, fe_degree_T, n_q_points_1d, 1, Number>   phi_m(data, true, EquationData::THETA_INDEX_DOF),
                                                                     phi_p(data, false, EquationData::THETA_INDEX_DOF),
                                                                     phi_theta_curr_m(data, true, EquationData::THETA_INDEX_DOF),
                                                                     phi_theta_curr_p(data, false, EquationData::THETA_INDEX_DOF);
      FEFaceEvaluation<dim, fe_degree_u, n_q_points_1d, dim, Number> phi_u_curr_m(data, true, EquationData::U_INDEX_DOF),
                                                                     phi_u_curr_p(data, false, EquationData::U_INDEX_DOF);
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        const auto coef_jump = C_T*0.5*(std::abs((phi_m.get_normal_vector(0) * phi_m.inverse_jacobian(0))[dim - 1]) +
                                        std::abs((phi_p.get_normal_vector(0) * phi_p.inverse_jacobian(0))[dim - 1])); /*--- Jump constant for IP ---*/
        ...
        for(unsigned int q = 0; q < phi_m.n_q_points; ++q) {
          ...
          const auto& avg_diff_flux_curr = 0.5*(diff_tensor_curr_m*grad_theta_curr_m +
                                                diff_tensor_curr_p*grad_theta_curr_p);

          /*--- Consider also jump penalization contribution ---*/
          const auto& theta_curr_m    = phi_theta_curr_m.get_value(q);
          const auto& theta_curr_p    = phi_theta_curr_p.get_value(q);
          const auto& avg_kappa_curr  = 2.0/(1.0/kappa_curr_m + 1.0/kappa_curr_p);
          const auto& jump_theta_curr = theta_curr_m - theta_curr_p;

          phi_m.submit_value(a21_tilde*dt*(scalar_product(avg_diff_flux_curr, n_minus) -
                                           coef_jump*avg_kappa_curr*jump_theta_curr), q);
          phi_p.submit_value(-a21_tilde*dt*(scalar_product(avg_diff_flux_curr, n_minus) -
                                            coef_jump*avg_kappa_curr*jump_theta_curr), q);
        }
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        phi_u_s_2_m.reinit(face);
        phi_u_s_2_m.gather_evaluate(src[2], EvaluationFlags::gradients);
        phi_u_s_2_p.reinit(face);
        phi_u_s_2_p.gather_evaluate(src[2], EvaluationFlags::gradients);
        phi_theta_s_2_m.reinit(face);
        phi_theta_s_2_m.gather_evaluate(src[3], EvaluationFlags::values | EvaluationFlags::gradients);
        phi_theta_s_2_p.reinit(face);
        phi_theta_s_2_p.gather_evaluate(src[3], EvaluationFlags::values | EvaluationFlags::gradients);
        ...
        for(unsigned int q = 0; q < phi_m.n_q_points; ++q) {
          ...
          const auto& kappa_s_2_p = EquationData::l_mixing*EquationData::l_mixing*std::sqrt(mod_squared_grad_uz_s_2_p)*
                                    std::pow(1.0 + b_p*std::abs(Ri_s_2_p), beta_p);
          Tensor<2, dim, VectorizedArray<Number>> diff_tensor_s_2_m,
                                                  diff_tensor_s_2_p;
          diff_tensor_s_2_m = 0;
          diff_tensor_s_2_m[dim - 1][dim - 1] = kappa_s_2_m;
          diff_tensor_s_2_p = 0;
          diff_tensor_s_2_p[dim - 1][dim - 1] = kappa_s_2_p;
          const auto& avg_diff_flux_s_2 = 0.5*(diff_tensor_s_2_m*grad_theta_s_2_m +
                                               diff_tensor_s_2_p*grad_theta_s_2_p);
          ...
          const auto& theta_s_2_m      = phi_theta_s_2_m.get_value(q);
          const auto& theta_s_2_p      = phi_theta_s_2_p.get_value(q);
          const auto& avg_kappa_s_2    = 2.0/(1.0/kappa_s_2_m + 1.0/kappa_s_2_p);
          const auto& jump_theta_s_2   = theta_s_2_m - theta_s_2_p;

          phi_m.submit_value(a31_tilde*dt*(scalar_product(avg_diff_flux_curr, n_minus) -
                                           coef_jump*avg_kappa_curr*jump_theta_curr) +
                             a32_tilde*dt*(scalar_product(avg_diff_flux_s_2, n_minus) -
                                           coef_jump*avg_kappa_s_2*jump_theta_s_2), q);
          phi_p.submit_value(-a31_tilde*dt*(scalar_product(avg_diff_flux_curr, n_minus) -
                                            coef_jump*avg_kappa_curr*jump_theta_curr)
                             -a32_tilde*dt*(scalar_product(avg_diff_flux_s_2, n_minus) -
                                            coef_jump*avg_kappa_s_2*jump_theta_s_2), q);
        }
  ...
  assemble_face_term_temperature(const MatrixFree<dim, Number>&               data,
    ...
    for(unsigned int face = face_range.first; face < face_range.second; ++face) {
      ...
      for(unsigned int q = 0; q < phi_m.n_q_points; ++q) {
        ...
        diff_tensor_curr_p = 0;
        diff_tensor_curr_p[dim - 1][dim - 1] = kappa_curr_p;
        const auto& avg_diff_flux = 0.5*(diff_tensor_curr_m*phi_m.get_gradient(q) +
        ...
        phi_m.submit_value(coeff*(-scalar_product(avg_diff_flux, n_minus) + coef_jump*avg_kappa_curr*jump_theta), q);
        phi_p.submit_value(-coeff*(-scalar_product(avg_diff_flux, n_minus) + coef_jump*avg_kappa_curr*jump_theta), q);
      }
  ...
  assemble_diagonal_face_term_velocity(const MatrixFree<dim, Number>&               data,
    ...
    for(unsigned int face = face_range.first; face < face_range.second; ++face) {
      ...
      /*--- Loop over all dofs ---*/
      for(unsigned int i = 0; i < phi_m.dofs_per_component; ++i) {
        for(unsigned int j = 0; j < phi_m.dofs_per_component; ++j) {
