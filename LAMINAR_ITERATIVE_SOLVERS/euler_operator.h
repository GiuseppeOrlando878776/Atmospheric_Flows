...
namespace Atmospheric_Flow {
  ...
  private:
    ...
    /**
     * Assemble cell contribution continuity equation
     * @param data matrix-free infrastructure with all data
     * @return dst destination vector
    ...
    /**
     * Assemble cell contribution 'velocity' operator momentum equation
     * @param data matrix-free infrastructure with all data
     * @return dst destination vector
     * @param src vector to which aply the operator
     * @param cell_range range of cells to be locally assembled
    ...
    /**
     * Assemble diagonal mass matrix continuity equation
     * @param data matrix-free infrastructure with all data
     * @return dst destination vector
     * @param src vector to which aply the operator
     * @param cell_range range of cells to be locally assembled
    ...
    /**
     * Assemble diagonal 'A' matrix momentum equation
     * @param data matrix-free infrastructure with all data
     * @return dst destination vector
     * @param src vector to which aply the operator
     * @param cell_range range of cells to be locally assembled
    ...
    /**
     * Assemble diagonal 'D' matrix energy equation
     * @param data matrix-free infrastructure with all data
     * @return dst destination vector
     * @param src vector to which aply the operator
     * @param cell_range range of cells to be locally assembled
  ...
  //
  template<unsigned dim,
           unsigned fe_degree_rho, unsigned fe_degree_p, unsigned fe_degree_u,
           unsigned n_q_points_1d, unsigned n_q_points_1d_boundary,
           typename Vec>
  inline DEAL_II_ALWAYS_INLINE
  void EULEROperator<dim,
                     fe_degree_rho, fe_degree_p, fe_degree_u,
                     n_q_points_1d, n_q_points_1d_boundary,
                     Vec>::
  set_n_stages(const unsigned n_stages_) {
    n_stages = n_stages_;
  }
      ...
      for(unsigned cell = cell_range.first; cell < cell_range.second; ++cell) {
        ...
        for(const unsigned q : phi.quadrature_point_indices()) {
          ...
          const auto& rhoE_old = inv_gamma_m1*pres_old
                               + rho_old*(static_cast<Number>(0.5)*Ma2*scalar_product(u_old, u_old));
      ...
      for(unsigned face = face_range.first; face < face_range.second; ++face) {
        ...
        for(const unsigned q : phi_m.quadrature_point_indices()) {
          ...
          flux_num += a_tilde[IMEX_stage - 1][IMEX_stage - 1]*dt*
                      (static_cast<Number>(0.5)*lambda_fixed_s*jump_rho_e_fixed_s);
  ...
  apply_add(Vec& dst, const Vec& src) const {
    ...
    if(Euler_stage == EquationData::RHO_INDEX_SYSTEM) {
      ...
      if(IMEX_stage <= n_stages) {
        ...
        SolverControl solver_control(10000, static_cast<Number>(1e-12)*tmp_1.l2_norm());
