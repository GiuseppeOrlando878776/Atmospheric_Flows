...
namespace Atmospheric_Flow {
  ...
  assemble_rhs_cell_term_momentum(const MatrixFree<dim, Number>&               data,
    ...
    if(IMEX_stage == 2) {
      ...
      FEEvaluation<dim, fe_degree_p, n_q_points_1d, 1, Number>   phi_pres_prime_old(data, EquationData::P_INDEX_DOF);
      FEEvaluation<dim, fe_degree_rho, n_q_points_1d, 1, Number> phi_rho_old(data, EquationData::RHO_INDEX_DOF),
                                                                 phi_rho_prime_old(data, EquationData::RHO_INDEX_DOF),
                                                                 phi_rho_prime_s_2(data, EquationData::RHO_INDEX_DOF);
      ...
      for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
        ...
        phi_pres_prime_old.gather_evaluate(src[2], EvaluationFlags::values);

        phi_rho_prime_old.reinit(cell);
        phi_rho_prime_old.gather_evaluate(src[3], EvaluationFlags::values);

        phi_rho_prime_s_2.reinit(cell);
        phi_rho_prime_s_2.gather_evaluate(src[4], EvaluationFlags::values);
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          Tensor<2, dim, VectorizedArray<Number>> p_prime_n_times_identity;
          for(unsigned int d = 0; d < dim; ++d) {
            p_prime_n_times_identity[d][d] = pres_prime_old;
          }

          /*--- Evaluate perturbation of the density ---*/
          const auto& rho_prime_old = phi_rho_prime_old.get_value(q);
          const auto& rho_prime_s_2 = phi_rho_prime_s_2.get_value(q);

          phi.submit_value(rho_old*u_old -
                           a21_tilde*dt*(rho_prime_old*e_k/(Fr*Fr)) -
                           a22_tilde*dt*(rho_prime_s_2*e_k/(Fr*Fr)), q);
          phi.submit_gradient(a21*dt*(rho_old*tensor_product_u_n) +
                              a21_tilde*dt*(p_prime_n_times_identity/(Ma*Ma)), q);
        }
      ...
      FEEvaluation<dim, fe_degree_u, n_q_points_1d, dim, Number> phi(data, EquationData::U_INDEX_DOF),
                                                                 ...
                                                                 phi_u_s_2(data, EquationData::U_INDEX_DOF);
      FEEvaluation<dim, fe_degree_p, n_q_points_1d, 1, Number>   phi_pres_prime_old(data, EquationData::P_INDEX_DOF),
                                                                 phi_pres_prime_s_2(data, EquationData::P_INDEX_DOF);
      FEEvaluation<dim, fe_degree_rho, n_q_points_1d, 1, Number> phi_rho_old(data, EquationData::RHO_INDEX_DOF),
                                                                 phi_rho_s_2(data, EquationData::RHO_INDEX_DOF),
                                                                 phi_rho_prime_old(data, EquationData::RHO_INDEX_DOF),
                                                                 phi_rho_prime_s_2(data, EquationData::RHO_INDEX_DOF),
                                                                 phi_rho_prime_s_3(data, EquationData::RHO_INDEX_DOF);
      ...
      for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
        ...
        phi_pres_prime_s_2.reinit(cell);
        phi_pres_prime_s_2.gather_evaluate(src[5], EvaluationFlags::values);

        phi_rho_prime_old.reinit(cell);
        phi_rho_prime_old.gather_evaluate(src[6], EvaluationFlags::values);
        phi_rho_prime_s_2.reinit(cell);
        phi_rho_prime_s_2.gather_evaluate(src[7], EvaluationFlags::values);
        phi_rho_prime_s_3.reinit(cell);
        phi_rho_prime_s_3.gather_evaluate(src[8], EvaluationFlags::values);
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          const auto& pres_prime_s_2       = phi_pres_prime_s_2.get_value(q);
          Tensor<2, dim, VectorizedArray<Number>> p_prime_s_2_times_identity;
          for(unsigned int d = 0; d < dim; ++d) {
            p_prime_s_2_times_identity[d][d] = pres_prime_s_2;
          }

          /*--- Evaluate perturbation of the density ---*/
          const auto& rho_prime_old = phi_rho_prime_old.get_value(q);
          const auto& rho_prime_s_2 = phi_rho_prime_s_2.get_value(q);
          const auto& rho_prime_s_3 = phi_rho_prime_s_3.get_value(q);

          phi.submit_value(rho_old*u_old -
                           a31_tilde*dt*(rho_prime_old*e_k/(Fr*Fr)) -
                           a32_tilde*dt*(rho_prime_s_2*e_k/(Fr*Fr)) -
                           a33_tilde*dt*(rho_prime_s_3*e_k/(Fr*Fr)), q);
          phi.submit_gradient(a31*dt*(rho_old*tensor_product_u_n) +
                              a31_tilde*dt*(p_prime_n_times_identity/(Ma*Ma)) +
                              a32*dt*(rho_s_2*tensor_product_u_s_2) +
                              a32_tilde*dt*(p_prime_s_2_times_identity/(Ma*Ma)), q);
        }
      ...
      FEEvaluation<dim, fe_degree_p, n_q_points_1d, 1, Number>   phi_pres_prime_old(data, EquationData::P_INDEX_DOF),
                                                                 phi_pres_prime_s_2(data, EquationData::P_INDEX_DOF),
                                                                 phi_pres_prime_s_3(data, EquationData::P_INDEX_DOF);
      FEEvaluation<dim, fe_degree_rho, n_q_points_1d, 1, Number> phi_rho_old(data, EquationData::RHO_INDEX_DOF),
                                                                 phi_rho_s_2(data, EquationData::RHO_INDEX_DOF),
                                                                 phi_rho_s_3(data, EquationData::RHO_INDEX_DOF),
                                                                 phi_rho_prime_old(data, EquationData::RHO_INDEX_DOF),
      ...
      for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
        ...
        phi_pres_prime_s_3.reinit(cell);
        phi_pres_prime_s_3.gather_evaluate(src[8], EvaluationFlags::values);

        phi_rho_prime_old.reinit(cell);
        phi_rho_prime_old.gather_evaluate(src[9], EvaluationFlags::values);
        phi_rho_prime_s_2.reinit(cell);
        phi_rho_prime_s_2.gather_evaluate(src[10], EvaluationFlags::values);
        phi_rho_prime_s_3.reinit(cell);
        phi_rho_prime_s_3.gather_evaluate(src[11], EvaluationFlags::values);
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          const auto& pres_prime_s_3       = phi_pres_prime_s_3.get_value(q);
          Tensor<2, dim, VectorizedArray<Number>> p_prime_s_3_times_identity;
          for(unsigned int d = 0; d < dim; ++d) {
            p_prime_s_3_times_identity[d][d] = pres_prime_s_3;
          }
          ...
          phi.submit_value(rho_old*u_old -
                           b1*dt*(rho_prime_old*e_k/(Fr*Fr)) -
                           b2*dt*(rho_prime_s_2*e_k/(Fr*Fr)) -
                           b3*dt*(rho_prime_s_3*e_k/(Fr*Fr)), q);
          phi.submit_gradient(b1*dt*(rho_old*tensor_product_u_n) +
                              b1*dt*(p_prime_n_times_identity/(Ma*Ma)) +
                              b2*dt*(rho_s_2*tensor_product_u_s_2) +
                              b2*dt*(p_prime_s_2_times_identity/(Ma*Ma)) +
                              b3*dt*(rho_s_3*tensor_product_u_s_3) +
                              b3*dt*(p_prime_s_3_times_identity/(Ma*Ma)), q);
        }
  ...
  assemble_rhs_face_term_momentum(const MatrixFree<dim, Number>&               data,
    ...
    if(IMEX_stage == 2) {
      ...
      FEFaceEvaluation<dim, fe_degree_u, n_q_points_1d, dim, Number> phi_m(data, true, EquationData::U_INDEX_DOF),
                                                                     ...
                                                                     phi_u_old_p(data, false, EquationData::U_INDEX_DOF);
      FEFaceEvaluation<dim, fe_degree_p, n_q_points_1d, 1, Number>   phi_pres_prime_old_m(data, true, EquationData::P_INDEX_DOF),
                                                                     phi_pres_prime_old_p(data, false, EquationData::P_INDEX_DOF);
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        phi_pres_prime_old_m.reinit(face);
        phi_pres_prime_old_m.gather_evaluate(src[2], EvaluationFlags::values);
        phi_pres_prime_old_p.reinit(face);
        phi_pres_prime_old_p.gather_evaluate(src[2], EvaluationFlags::values);
        ...
        for(unsigned int q = 0; q < phi_m.n_q_points; ++q) {
          ...
          const auto& pres_prime_old_m       = phi_pres_prime_old_m.get_value(q);
          const auto& pres_prime_old_p       = phi_pres_prime_old_p.get_value(q);

          const auto& avg_pres_prime_old     = 0.5*(pres_prime_old_m + pres_prime_old_p);
          ...
          phi_m.submit_value(-a21*dt*(avg_tensor_product_u_n*n_minus +
                             ...
                             -a21_tilde*dt*(avg_pres_prime_old/(Ma*Ma)*n_minus), q);
      ...
      FEFaceEvaluation<dim, fe_degree_p, n_q_points_1d, 1, Number>   phi_pres_prime_old_m(data, true, EquationData::P_INDEX_DOF),
                                                                     phi_pres_prime_old_p(data, false, EquationData::P_INDEX_DOF),
                                                                     phi_pres_prime_s_2_m(data, true, EquationData::P_INDEX_DOF),
                                                                     phi_pres_prime_s_2_p(data, false, EquationData::P_INDEX_DOF);
      FEFaceEvaluation<dim, fe_degree_rho, n_q_points_1d, 1, Number> phi_rho_old_m(data, true, EquationData::RHO_INDEX_DOF),
                                                                     phi_rho_old_p(data, false, EquationData::RHO_INDEX_DOF),
                                                                     phi_rho_s_2_m(data, true, EquationData::RHO_INDEX_DOF),
                                                                     phi_rho_s_2_p(data, false, EquationData::RHO_INDEX_DOF);
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        phi_pres_prime_s_2_m.reinit(face);
        phi_pres_prime_s_2_m.gather_evaluate(src[5], EvaluationFlags::values);
        phi_pres_prime_s_2_p.reinit(face);
        phi_pres_prime_s_2_p.gather_evaluate(src[5], EvaluationFlags::values);
        ...
        for(unsigned int q = 0; q < phi_m.n_q_points; ++q) {
          ...
          const auto& pres_prime_s_2_m         = phi_pres_prime_s_2_m.get_value(q);
          const auto& pres_prime_s_2_p         = phi_pres_prime_s_2_p.get_value(q);

          const auto& avg_pres_prime_s_2       = 0.5*(pres_prime_s_2_m + pres_prime_s_2_p);
          ...
          phi_m.submit_value(-a31*dt*(avg_tensor_product_u_n*n_minus +
                                      0.5*lambda_old*jump_rhou_old)
                             -a31_tilde*dt*(avg_pres_prime_old/(Ma*Ma)*n_minus)
                             -a32*dt*(avg_tensor_product_u_s_2*n_minus +
                                      0.5*lambda_s_2*jump_rhou_s_2)
                             -a32_tilde*dt*(avg_pres_prime_s_2/(Ma*Ma)*n_minus), q);
      ...
      FEFaceEvaluation<dim, fe_degree_p, n_q_points_1d, 1, Number>   phi_pres_prime_old_m(data, true, EquationData::P_INDEX_DOF),
                                                                     ...
                                                                     phi_pres_prime_s_2_m(data, true, EquationData::P_INDEX_DOF),
                                                                     phi_pres_prime_s_2_p(data, false, EquationData::P_INDEX_DOF),
                                                                     phi_pres_prime_s_3_m(data, true, EquationData::P_INDEX_DOF),
                                                                     phi_pres_prime_s_3_p(data, false, EquationData::P_INDEX_DOF);
      FEFaceEvaluation<dim, fe_degree_rho, n_q_points_1d, 1, Number> phi_rho_old_m(data, true, EquationData::RHO_INDEX_DOF),
                                                                     phi_rho_old_p(data, false, EquationData::RHO_INDEX_DOF),
                                                                     phi_rho_s_2_m(data, true, EquationData::RHO_INDEX_DOF),
                                                                     phi_rho_s_2_p(data, false, EquationData::RHO_INDEX_DOF),
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        phi_pres_prime_s_3_m.reinit(face);
        phi_pres_prime_s_3_m.gather_evaluate(src[8], EvaluationFlags::values);
        phi_pres_prime_s_3_p.reinit(face);
        phi_pres_prime_s_3_p.gather_evaluate(src[8], EvaluationFlags::values);
        ...
        for(unsigned int q = 0; q < phi_m.n_q_points; ++q) {
          ...
          const auto& pres_prime_s_3_m         = phi_pres_prime_s_3_m.get_value(q);
          const auto& pres_prime_s_3_p         = phi_pres_prime_s_3_p.get_value(q);

          const auto& avg_pres_prime_s_3       = 0.5*(pres_prime_s_3_m + pres_prime_s_3_p);
          ...
          phi_m.submit_value(-b1*dt*(avg_tensor_product_u_n*n_minus +
                                     0.5*lambda_old*jump_rhou_old)
                             -b1*dt*(avg_pres_prime_old/(Ma*Ma)*n_minus)
                             -b2*dt*(avg_tensor_product_u_s_2*n_minus +
                                     0.5*lambda_s_2*jump_rhou_s_2)
                             -b2*dt*(avg_pres_prime_s_2/(Ma*Ma)*n_minus)
                             -b3*dt*(avg_tensor_product_u_s_3*n_minus +
                                     0.5*lambda_s_3*jump_rhou_s_3)
                             -b3*dt*(avg_pres_prime_s_3/(Ma*Ma)*n_minus), q);
  ...
  assemble_rhs_boundary_term_momentum(const MatrixFree<dim, Number>&               data,
    ...
    if(IMEX_stage == 2) {
      ...
      FEFaceEvaluation<dim, fe_degree_p, n_q_points_1d_boundary, 1, Number>   phi_pres_prime_old(data, true, EquationData::P_INDEX_DOF, 1);

      /*--- Loop over all boundary faces ---*/
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        phi_pres_prime_old.reinit(face);
        phi_pres_prime_old.gather_evaluate(src[2], EvaluationFlags::values);
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          /*--- Compute the quantities at the previous step ---*/
          const auto& pres_prime_old     = phi_pres_prime_old.get_value(q);
          const auto& pres_prime_old_D   = pres_prime_old;

          const auto& avg_pres_prime_old = 0.5*(pres_prime_old + pres_prime_old_D);

          phi.submit_value(-a21_tilde*dt*(avg_pres_prime_old/(Ma*Ma)*n_minus), q);
        }
      ...
      FEFaceEvaluation<dim, fe_degree_u, n_q_points_1d_boundary, dim, Number> phi(data, true, EquationData::U_INDEX_DOF, 1);
      FEFaceEvaluation<dim, fe_degree_p, n_q_points_1d_boundary, 1, Number>   phi_pres_prime_old(data, true, EquationData::P_INDEX_DOF, 1),
                                                                              phi_pres_prime_s_2(data, true, EquationData::P_INDEX_DOF, 1);
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        phi_pres_prime_s_2.reinit(face);
        phi_pres_prime_s_2.gather_evaluate(src[5], EvaluationFlags::values);
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          /*--- Compute the quantities at the previous stage ---*/
          const auto& pres_prime_s_2     = phi_pres_prime_s_2.get_value(q);
          const auto& pres_prime_s_2_D   = pres_prime_s_2;

          const auto& avg_pres_prime_s_2 = 0.5*(pres_prime_s_2 + pres_prime_s_2_D);

          phi.submit_value(-a31_tilde*dt*(avg_pres_prime_old/(Ma*Ma)*n_minus)
                           -a32_tilde*dt*(avg_pres_prime_s_2/(Ma*Ma)*n_minus), q);
        }
      ...
      FEFaceEvaluation<dim, fe_degree_p, n_q_points_1d_boundary, 1, Number>   phi_pres_prime_old(data, true, EquationData::P_INDEX_DOF, 1),
                                                                              phi_pres_prime_s_2(data, true, EquationData::P_INDEX_DOF, 1),
                                                                              phi_pres_prime_s_3(data, true, EquationData::P_INDEX_DOF, 1);
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        phi_pres_prime_s_3.reinit(face);
        phi_pres_prime_s_3.gather_evaluate(src[8], EvaluationFlags::values);
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          const auto& pres_prime_s_3     = phi_pres_prime_s_3.get_value(q);
          const auto& pres_prime_s_3_D   = pres_prime_s_3;

          const auto& avg_pres_prime_s_3 = 0.5*(pres_prime_s_3 + pres_prime_s_3_D);

          phi.submit_value(-b1*dt*(avg_pres_prime_old/(Ma*Ma)*n_minus)
                           -b2*dt*(avg_pres_prime_s_2/(Ma*Ma)*n_minus)
                           -b3*dt*(avg_pres_prime_s_3/(Ma*Ma)*n_minus), q);
        }
  ...
  assemble_boundary_term_pressure(const MatrixFree<dim, Number>&               data,
    ...
    for(unsigned int face = face_range.first; face < face_range.second; ++face) {
      ...
      for(unsigned int q = 0; q < phi.n_q_points; ++q) {
        ...
        const auto& pres_prime_fixed_D = phi_src.get_value(q);

        phi.submit_value(coeff*dt*((0.5*(phi_src.get_value(q) + pres_prime_fixed_D))/(Ma*Ma)*n_minus), q);
      }
  ...
  assemble_rhs_cell_term_energy(const MatrixFree<dim, Number>&               data,
    ...
    if(IMEX_stage == 2) {
      ...
      FEEvaluation<dim, fe_degree_p, n_q_points_1d, 1, Number>   phi(data, EquationData::P_INDEX_DOF),
                                                                 ...
                                                                 phi_pres_prime_old(data, EquationData::P_INDEX_DOF);
      FEEvaluation<dim, fe_degree_u, n_q_points_1d, dim, Number> phi_u_old(data, EquationData::U_INDEX_DOF),
      ...
      for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
        ...
        phi_pres_prime_old.reinit(cell);
        phi_pres_prime_old.gather_evaluate(src[6], EvaluationFlags::values);
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          /*--- Compute perturbation of old pressure ---*/
          const auto& pres_prime_old = phi_pres_prime_old.get_value(q);

          phi.submit_value(1.0/(EquationData::Cp_Cv - 1.0)*pres_prime_old +
      ...
      for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
        ...
        phi_pres_prime_old.reinit(cell);
        phi_pres_prime_old.gather_evaluate(src[9], EvaluationFlags::values);
      ...
      for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
        ...
        phi_pres_prime_old.reinit(cell);
        phi_pres_prime_old.gather_evaluate(src[11], EvaluationFlags::values);
  ...
  assemble_rhs_face_term_energy(const MatrixFree<dim, Number>&               data,
    ...
    if(IMEX_stage == 2) {
      ...
      FEFaceEvaluation<dim, fe_degree_p, n_q_points_1d, 1, Number>   phi_m(data, true, EquationData::P_INDEX_DOF),
                                                                     ...
                                                                     phi_pres_prime_fixed_m(data, true, EquationData::P_INDEX_DOF),
                                                                     phi_pres_prime_fixed_p(data, false, EquationData::P_INDEX_DOF),
                                                                     phi_pres_prime_old_m(data, true, EquationData::P_INDEX_DOF),
                                                                     phi_pres_prime_old_p(data, false, EquationData::P_INDEX_DOF);
      FEFaceEvaluation<dim, fe_degree_u, n_q_points_1d, dim, Number> phi_u_old_m(data, true, EquationData::U_INDEX_DOF),
                                                                     phi_u_old_p(data, false, EquationData::U_INDEX_DOF),
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        phi_pres_prime_fixed_m.reinit(face);
        phi_pres_prime_fixed_m.gather_evaluate(src[5], EvaluationFlags::values);
        phi_pres_prime_fixed_p.reinit(face);
        phi_pres_prime_fixed_p.gather_evaluate(src[5], EvaluationFlags::values);

        phi_pres_prime_old_m.reinit(face);
        phi_pres_prime_old_m.gather_evaluate(src[6], EvaluationFlags::values);
        phi_pres_prime_old_p.reinit(face);
        phi_pres_prime_old_p.gather_evaluate(src[6], EvaluationFlags::values);
        ...
        for(unsigned int q = 0; q < phi_m.n_q_points; ++q) {
          ...
          const auto& pres_prime_old_m     = phi_pres_prime_old_m.get_value(q);
          const auto& pres_prime_old_p     = phi_pres_prime_old_p.get_value(q);
          const auto& jump_rho_e_prime_old = 1.0/(EquationData::Cp_Cv - 1.0)*(pres_prime_old_m - pres_prime_old_p);
          ...
          const auto& pres_prime_fixed_m     = phi_pres_prime_fixed_m.get_value(q);
          const auto& pres_prime_fixed_p     = phi_pres_prime_fixed_p.get_value(q);

          const auto& lambda_fixed           = compute_lambda(u_fixed_m, u_fixed_p, n_minus);
          const auto& jump_rho_e_prime_fixed = 1.0/(EquationData::Cp_Cv - 1.0)*(pres_prime_fixed_m - pres_prime_fixed_p);

          phi_m.submit_value(-a21*dt*(Ma*Ma*(scalar_product(avg_kinetic_old, n_minus) + 0.5*lambda_old*jump_rho_kin_old))
                             -a21_tilde*dt*(scalar_product(avg_enthalpy_old, n_minus) + 0.5*lambda_old*jump_rho_e_prime_old)
                             -a22_tilde*dt*(0.5*lambda_fixed*jump_rho_e_prime_fixed), q);
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        phi_pres_prime_fixed_m.reinit(face);
        phi_pres_prime_fixed_m.gather_evaluate(src[8], EvaluationFlags::values);
        phi_pres_prime_fixed_p.reinit(face);
        phi_pres_prime_fixed_p.gather_evaluate(src[8], EvaluationFlags::values);

        phi_pres_prime_old_m.reinit(face);
        phi_pres_prime_old_m.gather_evaluate(src[9], EvaluationFlags::values);
        phi_pres_prime_old_p.reinit(face);
        phi_pres_prime_old_p.gather_evaluate(src[9], EvaluationFlags::values);
        phi_pres_prime_s_2_m.reinit(face);
        phi_pres_prime_s_2_m.gather_evaluate(src[10], EvaluationFlags::values);
        phi_pres_prime_s_2_p.reinit(face);
        phi_pres_prime_s_2_p.gather_evaluate(src[10], EvaluationFlags::values);
        ...
        for(unsigned int q = 0; q < phi_m.n_q_points; ++q) {
          ...
          const auto& pres_prime_s_2_m     = phi_pres_prime_s_2_m.get_value(q);
          const auto& pres_prime_s_2_p     = phi_pres_prime_s_2_p.get_value(q);
          const auto& jump_rho_e_prime_s_2 = 1.0/(EquationData::Cp_Cv - 1.0)*(pres_prime_s_2_m - pres_prime_s_2_p);
          ...
          phi_m.submit_value(-a31*dt*(Ma*Ma*(scalar_product(avg_kinetic_old, n_minus) + 0.5*lambda_old*jump_rho_kin_old))
                             -a31_tilde*dt*(scalar_product(avg_enthalpy_old, n_minus) + 0.5*lambda_old*jump_rho_e_prime_old)
                             -a32*dt*(Ma*Ma*(scalar_product(avg_kinetic_s_2, n_minus) + 0.5*lambda_s_2*jump_rho_kin_s_2))
                             -a32_tilde*dt*(scalar_product(avg_enthalpy_s_2, n_minus) + 0.5*lambda_s_2*jump_rho_e_prime_s_2)
                             -a33_tilde*dt*(0.5*lambda_fixed*jump_rho_e_prime_fixed), q);
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        phi_pres_prime_old_m.reinit(face);
        phi_pres_prime_old_m.gather_evaluate(src[11], EvaluationFlags::values);
        phi_pres_prime_old_p.reinit(face);
        phi_pres_prime_old_p.gather_evaluate(src[11], EvaluationFlags::values);
        phi_pres_prime_s_2_m.reinit(face);
        phi_pres_prime_s_2_m.gather_evaluate(src[12], EvaluationFlags::values);
        phi_pres_prime_s_2_p.reinit(face);
        phi_pres_prime_s_2_p.gather_evaluate(src[12], EvaluationFlags::values);
        phi_pres_prime_s_3_m.reinit(face);
        phi_pres_prime_s_3_m.gather_evaluate(src[13], EvaluationFlags::values);
        phi_pres_prime_s_3_p.reinit(face);
        phi_pres_prime_s_3_p.gather_evaluate(src[13], EvaluationFlags::values);
        ...
        for(unsigned int q = 0; q < phi_m.n_q_points; ++q) {
          ...
          const auto& lambda_old       = compute_lambda(u_old_m, u_old_p, n_minus);
          const auto& pres_prime_old_m = phi_pres_prime_old_m.get_value(q);
          const auto& rhoE_old_m       = 1.0/(EquationData::Cp_Cv - 1.0)*pres_prime_old_m
                                       + rho_old_m*(0.5*Ma*Ma*scalar_product(u_old_m, u_old_m));
          const auto& pres_prime_old_p = phi_pres_prime_old_p.get_value(q);
          const auto& rhoE_old_p       = 1.0/(EquationData::Cp_Cv - 1.0)*pres_prime_old_p
          ...
          const auto& lambda_s_2       = compute_lambda(u_s_2_m, u_s_2_p, n_minus);
          const auto& pres_prime_s_2_m = phi_pres_prime_s_2_m.get_value(q);
          const auto& rhoE_s_2_m       = 1.0/(EquationData::Cp_Cv - 1.0)*pres_prime_s_2_m
                                       + rho_s_2_m*(0.5*Ma*Ma*scalar_product(u_s_2_m, u_s_2_m));
          const auto& pres_prime_s_2_p = phi_pres_prime_s_2_p.get_value(q);
          const auto& rhoE_s_2_p       = 1.0/(EquationData::Cp_Cv - 1.0)*pres_prime_s_2_p
          ...
          const auto& lambda_s_3       = compute_lambda(u_s_3_m, u_s_3_p, n_minus);
          const auto& pres_prime_s_3_m = phi_pres_prime_s_3_m.get_value(q);
          const auto& rhoE_s_3_m       = 1.0/(EquationData::Cp_Cv - 1.0)*pres_prime_s_3_m
                                       + rho_s_3_m*(0.5*Ma*Ma*scalar_product(u_s_3_m, u_s_3_m));
          const auto& pres_prime_s_3_p = phi_pres_prime_s_3_p.get_value(q);
          const auto& rhoE_s_3_p       = 1.0/(EquationData::Cp_Cv - 1.0)*pres_prime_s_3_p
