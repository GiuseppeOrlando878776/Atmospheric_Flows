...
namespace Atmospheric_Flow {
  ...
  private:
    ...
    inline VectorizedArray<Number> compute_lambda(const Tensor<1, dim, VectorizedArray<Number>>& u_p,
                                                  const Tensor<1, dim, VectorizedArray<Number>>& u_m,
                                                  const Tensor<1, dim, VectorizedArray<Number>>& n_plus) const;
  ...
  EULEROperator<dim, fe_degree_u, fe_degree_rho, fe_degree_p,
                n_q_points_1d, n_q_points_1d_boundary, Vec>::
  compute_lambda(const Tensor<1, dim, VectorizedArray<Number>>& u_p,
                 const Tensor<1, dim, VectorizedArray<Number>>& u_m,
                 const Tensor<1, dim, VectorizedArray<Number>>& n_plus) const {
    return std::max(std::abs(scalar_product(u_p, n_plus)),
                    std::abs(scalar_product(u_m, n_plus)));
  }
  ...
  assemble_rhs_cell_term_density(const MatrixFree<dim, Number>&               data,
    ...
    if(IMEX_stage == 2) {
      ...
      FEEvaluation<dim, fe_degree_rho, n_q_points_1d, 1, Number> phi(data, 2),
                                                                 phi_rho_old(data, 2);
      FEEvaluation<dim, fe_degree_u, n_q_points_1d, dim, Number> phi_u_old(data, 0);
  ...
  assemble_rhs_face_term_density(const MatrixFree<dim, Number>&               data,
    ...
    if(IMEX_stage == 2) {
      ...
      FEFaceEvaluation<dim, fe_degree_rho, n_q_points_1d, 1, Number> phi_p(data, true, 2),
                                                                     phi_m(data, false, 2),
                                                                     phi_rho_old_p(data, true, 2),
                                                                     phi_rho_old_m(data, false, 2);
      FEFaceEvaluation<dim, fe_degree_u, n_q_points_1d, dim, Number> phi_u_old_p(data, true, 0),
  ...
  assemble_rhs_cell_term_momentum(const MatrixFree<dim, Number>&               data,
    ...
    if(IMEX_stage == 2) {
      ...
      FEEvaluation<dim, fe_degree_u, n_q_points_1d, dim, Number> phi(data, 0),
                                                                 phi_u_old(data, 0);
      FEEvaluation<dim, fe_degree_p, n_q_points_1d, 1, Number>   phi_pres_old(data, 1);
      FEEvaluation<dim, fe_degree_rho, n_q_points_1d, 1, Number> phi_rho_old(data, 2),
                                                                 phi_rho_tmp_2(data, 2);
      ...
      for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          phi.submit_value(rho_old*u_old -
                           ...
                           a22_tilde*dt*(rho_tmp_2*e_k/(Fr*Fr)), q);
      ...
      for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          Tensor<2, dim, VectorizedArray<Number>> p_tmp_2_times_identity;
          for(unsigned int d = 0; d < dim; ++d) {
          ...
          phi.submit_value(rho_old*u_old -
                           ...
                           a32_tilde*dt*(rho_tmp_2*e_k/(Fr*Fr)) -
          ...
          phi.submit_gradient(a31*dt*(rho_old*tensor_product_u_n) +
                              ...
                              a32*dt*(rho_tmp_2*tensor_product_u_tmp_2) +
                              a32_tilde*dt*(p_tmp_2_times_identity/(Ma*Ma)), q);
        }
      ...
      for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          Tensor<2, dim, VectorizedArray<Number>> p_tmp_3_times_identity;
          for(unsigned int d = 0; d < dim; ++d) {
          ...
          phi.submit_value(rho_old*u_old -
                           ...
                           b2*dt*(rho_tmp_2*e_k/(Fr*Fr)) -
                           b3*dt*(rho_tmp_3*e_k/(Fr*Fr)), q);
          phi.submit_gradient(b1*dt*(rho_old*tensor_product_u_n) +
                              b1*dt*(p_n_times_identity/(Ma*Ma)) +
                              b2*dt*(rho_tmp_2*tensor_product_u_tmp_2) +
                              b2*dt*(p_tmp_2_times_identity/(Ma*Ma)) +
                              b3*dt*(rho_tmp_3*tensor_product_u_tmp_3) +
                              b3*dt*(p_tmp_3_times_identity/(Ma*Ma)), q);
        }
  ...
  assemble_rhs_face_term_momentum(const MatrixFree<dim, Number>&               data,
    ...
    if(IMEX_stage == 2) {
      ...
      FEFaceEvaluation<dim, fe_degree_u, n_q_points_1d, dim, Number> phi_p(data, true, 0),
                                                                     ...
                                                                     phi_u_old_m(data, false, 0);
      FEFaceEvaluation<dim, fe_degree_p, n_q_points_1d, 1, Number>   phi_pres_old_p(data, true, 1),
                                                                     phi_pres_old_m(data, false, 1);
      FEFaceEvaluation<dim, fe_degree_rho, n_q_points_1d, 1, Number> phi_rho_old_p(data, true, 2),
                                                                     phi_rho_old_m(data, false, 2);
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi_p.n_q_points; ++q) {
          ...
          phi_p.submit_value(-a21*dt*(avg_tensor_product_u_n*n_plus +
                             ...
                             -a21_tilde*dt*(avg_pres_old/(Ma*Ma)*n_plus), q);
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi_p.n_q_points; ++q) {
          ...
          phi_p.submit_value(-a31*dt*(avg_tensor_product_u_n*n_plus +
                                      0.5*lambda_old*jump_rhou_old)
                             -a31_tilde*dt*(avg_pres_old/(Ma*Ma)*n_plus)
                             -a32*dt*(avg_tensor_product_u_tmp_2*n_plus +
                                      0.5*lambda_tmp_2*jump_rhou_tmp_2)
                             -a32_tilde*dt*(avg_pres_tmp_2/(Ma*Ma)*n_plus), q);
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi_p.n_q_points; ++q) {
          ...
          phi_p.submit_value(-b1*dt*(avg_tensor_product_u_n*n_plus +
                                     0.5*lambda_old*jump_rhou_old)
                             -b1*dt*(avg_pres_old/(Ma*Ma)*n_plus)
                             -b2*dt*(avg_tensor_product_u_tmp_2*n_plus +
                                     0.5*lambda_tmp_2*jump_rhou_tmp_2)
                             -b2*dt*(avg_pres_tmp_2/(Ma*Ma)*n_plus)
                             -b3*dt*(avg_tensor_product_u_tmp_3*n_plus +
                                     0.5*lambda_tmp_3*jump_rhou_tmp_3)
                             -b3*dt*(avg_pres_tmp_3/(Ma*Ma)*n_plus), q);
  ...
  assemble_rhs_boundary_term_momentum(const MatrixFree<dim, Number>&               data,
    ...
    if(IMEX_stage == 2) {
      /*--- We first start by declaring the suitable instances to read the available quantities. ---*/
      FEFaceEvaluation<dim, fe_degree_u, n_q_points_1d_boundary, dim, Number> phi(data, true, 0, 1);
      FEFaceEvaluation<dim, fe_degree_p, n_q_points_1d_boundary, 1, Number>   phi_pres_old(data, true, 1, 1);
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          phi.submit_value(-a21_tilde*dt*(avg_pres_old/(Ma*Ma)*n_plus), q);
        }
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          phi.submit_value(-a31_tilde*dt*(avg_pres_old/(Ma*Ma)*n_plus)
                           -a32_tilde*dt*(avg_pres_tmp_2/(Ma*Ma)*n_plus), q);
        }
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          phi.submit_value(-b1*dt*(avg_pres_old/(Ma*Ma)*n_plus)
                           -b2*dt*(avg_pres_tmp_2/(Ma*Ma)*n_plus)
                           -b3*dt*(avg_pres_tmp_3/(Ma*Ma)*n_plus), q);
        }
  ...
  assemble_cell_term_velocity(const MatrixFree<dim, Number>&               data,
    ...
    FEEvaluation<dim, fe_degree_u, n_q_points_1d, dim, Number> phi(data, 0);
    FEEvaluation<dim, fe_degree_rho, n_q_points_1d, 1, Number> phi_rho_for_fixed(data, 2);
  ...
  assemble_cell_term_pressure(const MatrixFree<dim, Number>&               data,
    ...
    FEEvaluation<dim, fe_degree_u, n_q_points_1d, dim, Number> phi(data, 0);
    FEEvaluation<dim, fe_degree_p, n_q_points_1d, 1, Number>   phi_src(data, 1);
  ...
  assemble_face_term_pressure(const MatrixFree<dim, Number>&               data,
    ...
    FEFaceEvaluation<dim, fe_degree_u, n_q_points_1d, dim, Number> phi_p(data, true, 0),
                                                                   phi_m(data, false, 0);
    FEFaceEvaluation<dim, fe_degree_p, n_q_points_1d, 1, Number>   phi_src_p(data, true, 1),
    ...
    for(unsigned int face = face_range.first; face < face_range.second; ++face) {
      ...
      for(unsigned int q = 0; q < phi_p.n_q_points; ++q) {
        ...
        phi_p.submit_value(coeff*dt*(avg_term/(Ma*Ma)*n_plus), q);
        phi_m.submit_value(-coeff*dt*(avg_term/(Ma*Ma)*n_plus), q);
      }
  ...
  assemble_boundary_term_pressure(const MatrixFree<dim, Number>&               data,
                                  ...
                                  const std::pair<unsigned int, unsigned int>& face_range) const {
    FEFaceEvaluation<dim, fe_degree_u, n_q_points_1d_boundary, dim, Number> phi(data, true, 0, 1);
    FEFaceEvaluation<dim, fe_degree_p, n_q_points_1d_boundary, 1, Number>   phi_src(data, true, 1, 1);
    ...
    for(unsigned int face = face_range.first; face < face_range.second; ++face) {
      ...
      for(unsigned int q = 0; q < phi.n_q_points; ++q) {
        ...
        phi.submit_value(coeff*dt*((0.5*(phi_src.get_value(q) + pres_fixed_D))/(Ma*Ma)*n_plus), q);
      }
  ...
  assemble_rhs_face_term_energy(const MatrixFree<dim, Number>&               data,
    ...
    if(IMEX_stage == 2) {
      /*--- We first start by declaring the suitable instances to read the available quantities. ---*/
      FEFaceEvaluation<dim, fe_degree_p, n_q_points_1d, 1, Number>   phi_p(data, true, 1),
                                                                     phi_m(data, false, 1),
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi_p.n_q_points; ++q) {
          ...
          const auto& lambda_fixed     = compute_lambda(u_fixed_p, u_fixed_m, n_plus);
          ...
          phi_p.submit_value(-a21*dt*(Ma*Ma*(scalar_product(avg_kinetic_old, n_plus) + 0.5*lambda_old*jump_rho_kin_old))
                             -a21_tilde*dt*(scalar_product(avg_enthalpy_old, n_plus) + 0.5*lambda_old*jump_rho_e_old)
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi_p.n_q_points; ++q) {
          ...
          phi_p.submit_value(-a31*dt*(Ma*Ma*(scalar_product(avg_kinetic_old, n_plus) + 0.5*lambda_old*jump_rho_kin_old))
                             ...
                             -a32_tilde*dt*(scalar_product(avg_enthalpy_tmp_2, n_plus) + 0.5*lambda_tmp_2*jump_rho_e_tmp_2)
  ...
  assemble_cell_term_enthalpy(const MatrixFree<dim, Number>&               data,
    ...
    FEEvaluation<dim, fe_degree_u, n_q_points_1d, dim, Number> phi_src(data, 0);
  ...
  assemble_face_term_enthalpy(const MatrixFree<dim, Number>&               data,
    ...
    FEFaceEvaluation<dim, fe_degree_p, n_q_points_1d, 1, Number>   phi_p(data, true, 1),
                                                                   ...
                                                                   phi_pres_fixed_p(data, true, 1),
                                                                   phi_pres_fixed_m(data, false, 1);
    FEFaceEvaluation<dim, fe_degree_u, n_q_points_1d, dim, Number> phi_src_p(data, true, 0),
